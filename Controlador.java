...
public class Controlador {
	...
	public int Resolver() {
		...
		if(opcion.equalsIgnoreCase("F")){
			...
			while((!opcion_pelea.equalsIgnoreCase("5")) && (valkyrie.getVida_valkyrie()>0) && (enemigos.get(num_enemigo).getNivel_de_resistencia()>0)) {
					...
					if (Math.abs(cx_enemigo - valkyrie.getPosicion_x()) < 5) {
						...
						for (Enemigo enemigo : enemigos) {
							if ((cx_enemigo == enemigo.coordenada_x) && (cy_enemigo == enemigo.coordenada_y) && (cz_enemigo == enemigo.coordenada_z)) {
								...
								num_enemigo = enemigo.getNumero_de_enemigo() - 1;
								break;
							}
						}
			...
			if(valkyrie.getVida_valkyrie()==0){
				opcion="T";
